...
class Config:
    ...
    def __init__(self, path_to_config) -> None:
        ...
        self.num_articles = config_json['total_articles']
...
class Crawler:
    ...
    def find_articles(self) -> None:
        ...
        for link in self.seed_urls:
            ...
            paragraphs = main_bs.find_all('div',
                                          {'class':
                                               'Listing_list__qKmtM'
                                           }
                                          )
            ...
            for each_par in paragraphs:
                ...
                for elem in ans:
                    ...
                    if not link or link in self.urls:
                        continue
                    self.urls.append('https://tass.ru/' + link)
...
class HTMLParser:
    ...
    def _fill_article_with_text(self, article_soup: BeautifulSoup) -> None:
        ...
        """
        article_body = article_soup.find('article').stripped_strings
        art_text = ' '.join(i for i in article_body)
        art_text = re.sub(r'.*\/ТАСС\/\.', '', art_text)
...
class CrawlerRecursive(Crawler):
    ...
    def __init__(self, config: Config) -> None:
        ...
        self.visited_urls = []
        self.urls = []
        self.path = Path(__file__).parent / 'tass_crawler_data.json'
        self.load_data()
...
def main_recursion() -> None:
    ...
    """
    configuration = Config(path_to_config='TASS_scrapper_config.json')
    prepare_environment('tmp/TASS_opk_articles')
    ...
    for i, full_url in enumerate(r_crawler.urls, start=1):
        ...
        if isinstance(article, dict):
            ...
            with open(Path(f"tmp/TASS_opk_articles/{article_txt_name}"), 'w', encoding='utf-8') as file:
